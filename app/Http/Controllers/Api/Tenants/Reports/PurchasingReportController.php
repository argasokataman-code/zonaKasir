...
class PurchasingReportController extends Controller
{
    public function __invoke(Request $request, PurchasingReportService $service): Response
    {
        ...
        try {
            $reportData = $service->generate($request->only(['start_date', 'end_date']));
            ...
            Log::error('Failed to generate purchasing report: ' . $e->getMessage(), [
                'exception' => $e,
            ]);

            abort(500, 'Failed to generate purchasing report');
        }
    }
}
