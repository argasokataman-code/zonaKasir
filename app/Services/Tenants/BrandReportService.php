    ...
    public function generate(array $data)
    {
        ...
        $sellings = Selling::query()
            ...
            ->with(
                ...
                'sellingDetails.product:id,name,sku,category_id,brand_id',
                'sellingDetails.product.brand:id,name'
            )
        ...
        foreach ($sellings as $selling) {
            foreach ($selling->sellingDetails as $detail) {
                ...
                $brandName = $detail->product->brand->name ?? 'No Brand';
                ...
                $brandData[$brandId]['total_qty'] += $detail->qty;
                $brandData[$brandId]['total_revenue'] += $detail->price;
                $brandData[$brandId]['total_cost'] += $detail->cost;
                $brandData[$brandId]['total_discount'] += ($detail->discount_price ?? 0);
            }
        }
        ...
        foreach ($brandData as $brand) {
            $revenue = $brand['total_revenue'] - $brand['total_discount'];
            $profit = $revenue - $brand['total_cost'];

            $reports[] = [
                'brand_name' => $brand['brand_name'],
                'total_qty' => $brand['total_qty'],
                'total_revenue' => $this->formatCurrency($brand['total_revenue']),
                'total_discount' => $this->formatCurrency($brand['total_discount']),
                'total_after_discount' => $this->formatCurrency($revenue),
                'total_cost' => $this->formatCurrency($brand['total_cost']),
                'total_profit' => $this->formatCurrency($profit),
            ];

            $totalQty += $brand['total_qty'];
            $totalRevenue += $brand['total_revenue'];
            $totalCost += $brand['total_cost'];
            $totalDiscount += $brand['total_discount'];
        }
