<x-filament-panels::page>
    ...
    <div
        x-data="{ showModal: false, selectedPlanId: null, selectedPlanName: '' }"
        @plan-upgrade.window="showModal = false"
    >
        @if($snapRedirectUrl)
                        ...
                        <div class="p-5 flex flex-col h-full">
                            ...
                            <div class="mt-auto pt-3 border-t border-gray-100">
                                ...
                                @else
                                    <button
                                        type="button"
                                        x-on:click="selectedPlanId = {{ $plan['id'] }}; selectedPlanName = '{{ $plan['name'] }}'; showModal = true"
        ...
        >
            ...
            <div
                class="relative bg-white rounded-[8px] shadow-xl w-full max-w-sm p-6"
                x-transition:enter="transition ease-out duration-200"
                x-transition:enter-start="opacity-0 scale-95"
                x-transition:enter-end="opacity-100 scale-100"
            >
                <h3 class="text-sm font-bold text-gray-900 mb-1">Pilih Siklus Pembayaran</h3>
                <p class="text-xs text-gray-500 mb-5">Pilih paket untuk <span class="font-bold text-gray-700" x-text="selectedPlanName"></span></p>

                <div class="space-y-3">
                    <button
                        type="button"
                        x-on:click="$wire.subscribePlan(selectedPlanId, 'monthly')"
                        class="w-full flex items-center justify-between p-3 border border-gray-200 rounded-[6px] hover:border-gray-900 hover:bg-gray-50 transition-all cursor-pointer group"
                    >
                        <div class="text-left">
                            <p class="text-sm font-bold text-gray-900">Bulanan</p>
                            <p class="text-[10px] text-gray-400 font-medium mt-0.5">Bayar setiap bulan</p>
                        </div>
                        <svg class="w-4 h-4 text-gray-400 group-hover:text-gray-900 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
                    </button>

                    @if(collect($plans)->contains(fn($p) => ($p['price_yearly'] ?? 0) > 0))
                    <button
                        type="button"
                        x-on:click="$wire.subscribePlan(selectedPlanId, 'yearly')"
                        class="w-full flex items-center justify-between p-3 border border-gray-200 rounded-[6px] hover:border-gray-900 hover:bg-gray-50 transition-all cursor-pointer group"
                    >
                        <div class="text-left">
                            <p class="text-sm font-bold text-gray-900">Tahunan</p>
                            <p class="text-[10px] text-gray-400 font-medium mt-0.5">Hemat lebih banyak</p>
                        </div>
                        <svg class="w-4 h-4 text-gray-400 group-hover:text-gray-900 transition-colors" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
                    </button>
                    @endif
                ...
                <button
                    ...
                    class="w-full mt-4 text-center text-[10px] font-bold text-gray-400 uppercase tracking-widest hover:text-gray-600 transition-colors cursor-pointer"
                >
                    Batal
                </button>
            </div>
