    ...
    <div class="flex justify-between items-center">
      ...
      <div class="flex items-center gap-1 shrink-0 ml-2">
        ...
        <button class="flex h-7 w-7 items-center justify-center rounded-lg bg-red-100 text-red-600 text-sm relative"
          x-on:click="$dispatch('confirm-modal', { id: 'confirm', title: '{{ __("Delete Item") }}', message: '{{ __("Remove") }} {{ addslashes($item->product?->name ?? __("item")) }} {{ __("from cart?") }}', opts: { action: 'confirm-delete-item', params: { id: {{ $item->product_id }} } } })"
