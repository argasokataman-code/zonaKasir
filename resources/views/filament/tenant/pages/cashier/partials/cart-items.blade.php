...
<div x-data="{ deleteModal: false, deleteName: '', deleteId: null }" @confirm-delete.window="deleteModal = true; deleteName = $event.detail.name; deleteId = $event.detail.id;" @confirm-delete-ok.window="$wire.deleteCart(deleteId); deleteModal = false;">
    ...
    <div class="flex justify-between items-center">
      ...
      <div class="flex items-center gap-1 shrink-0 ml-2">
        ...
        <button class="flex h-7 w-7 items-center justify-center rounded-lg bg-red-100 text-red-600 text-sm relative"
          x-on:click="$dispatch('confirm-delete', { id: {{ $item->product_id }}, name: '{{ addslashes($item->product?->name ?? __('item')) }}' })"
...
<div x-show="deleteModal" x-cloak
  ...
  <div class="mx-4 w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl dark:bg-gray-800"
    ...
    <div class="flex items-center gap-3 mb-4">
      ...
      <div>
        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('Delete Item') }}</h3>
        <p class="text-sm text-gray-500 dark:text-gray-400">{{ __('Remove this item from cart?') }}</p>
      </div>
    </div>
    <p class="mb-6 text-sm font-medium text-gray-700 dark:text-gray-300" x-text="deleteName"></p>
    <div class="flex gap-3">
      ...
      <button x-on:click="$dispatch('confirm-delete-ok')"
