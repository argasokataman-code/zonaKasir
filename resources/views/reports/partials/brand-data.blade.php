<div class="max-w-full">
  <div class="text-center space-y-2">
    <h1 class="text-3xl font-semibold">{{ __('Sales per Brand') }}</h1>
  ...
  <div class="overflow-x-auto">
    <x-table class="w-full text-sm">
      <x-table-header>
        <x-table-header-cell>{{ __('Brand') }}</x-table-header-cell>
      ...
      <tbody>
        @foreach($reports as $key => $report)
          <x-table-row>
            <x-table-cell class="whitespace-nowrap">{{ $report['brand_name'] }}</x-table-cell>
