...
// ─── Livewire Error Handler (toast instead of modal) ──────────
document.addEventListener('livewire:error', function (event) {
  ...
  if (detail && detail.response) {
    var status = detail.response.status;
    var messages = {
      419: 'Sesi telah habis. Silakan muat ulang halaman.',
      422: 'Data tidak valid. Periksa input kamu.',
      429: 'Terlalu banyak permintaan. Tunggu beberapa saat.',
      500: 'Kesalahan server. Tim kami sudah diberitahu.',
    };
    if (messages[status]) {
  ...
  // Show toast via Filament's notification system
  if (window.Livewire && window.Livewire.dispatch) {
    window.Livewire.dispatch('notify', [{ message: message, type: 'danger' }]);
  }
});

// ─── Global fetch error handler (for non-Livewire requests) ──
window.addEventListener('unhandledrejection', function (event) {
  var reason = event.reason;
  if (reason && reason.status && reason.status >= 400) {
    console.warn('[Error Handler]', reason.status, reason.statusText);
  }
});
